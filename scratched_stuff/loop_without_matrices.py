...
class Structure(object):
    ...
    def generate_edge_points(self, subdivison_weight_matrix):
        ...
        if len(idx_crease) > 0:
            ...
            vertex_weight_combined_1 = (1 - self.data['creases'][idx_crease].reshape(-1, 1)) * \
                                       np.array([0.375, 0.375, 0.125, 0.125])
            vertex_weight_combined_2 = (self.data['creases'][idx_crease].reshape(-1, 1)) * \
                                       np.array([0.5, 0.5, 0, 0])
            vertex_weight_combined = vertex_weight_combined_1 + vertex_weight_combined_2
            print(idx_crease)
        ...
        print(subdivison_weight_matrix.toarray())
    ...
    def main(self):
        ...
        print(np.nonzero(subdivision_weight_matrix.toarray()[7000]))
        self.data['vertices'] = subdivision_weight_matrix.dot(self.data['vertices'][:old_vertex_count])
        print(len(self.data['vertices']))
        print(old_vertex_count)
        ###############################################################################################################
